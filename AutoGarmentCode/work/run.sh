...
SAMPLE="${1:-10112_9583}"
SIM="${2:-sim}"
GENDER="${3:-${GENDER:-male}}"

# ---- 阶段开关 ----
DO_RENDER="${DO_RENDER:-0}"
DO_HYBRIK="${DO_HYBRIK:-0}"
DO_SMPLOBJ="${DO_SMPLOBJ:-0}"
DO_MEASURE="${DO_MEASURE:-0}"
DO_AGENT="${DO_AGENT:-0}"
...
if [ "$DO_HYBRIK" = "1" ]; then
  echo "[2/6] estimating SMPL parameters with HybrIK ..."
...
if [ "$DO_AGENT" = "1" ]; then
  ...
  echo "[5/6] skipping agent"
fi

# ---- 6. GarmentCode 生成样板 + 仿真 + 姿态驱动 + 指标 ----
if [ "$DO_GARMENT" = "1" ]; then
  echo "[6/6] generating, driving and evaluating garment (sim=$SIM) ...  [$(date '+%H:%M:%S')]"
  _sim_start=$(date +%s)
  ( cd "$PROJECT_ROOT" && $CONDA garmentcode python "$PROJECT_ROOT/work/garmentcode.py" \
        ...
        --sim "$SIM" \
        --target_pose "$NPZ_PATH" \
        --base_smpl_json "$HYBRIK_OUT_DIR/smpl.json" \
        --gender "$GENDER" )

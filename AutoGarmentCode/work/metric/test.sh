...
WORK_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"           # .../AutoGarmentCode/work
PROJECT_ROOT="$(cd "$WORK_DIR/.." && pwd)"         # .../AutoGarmentCode
...
if [ "$DO_DRIVE" = "1" ]; then
  ...
  $CONDA_NW python "$WORK_DIR/driven_garment_mesh.py" \
      --sample "$SAMPLE" \
      --gender "$GENDER"

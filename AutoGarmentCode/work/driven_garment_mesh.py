...
class WarpClothSimulator:
    ...
    def step(self, body_verts_np, dt=1.0/60.0, gravity_enabled=True):
        ...
        # 显式积分（SemiImplicit）对刚性弹簧稳定性差，必须把一帧拆成多个子步。
...
# ======================================
def get_args():
    parser = argparse.ArgumentParser(description='driven garmentcode to ground truth pose.')
...
if __name__ == "__main__":
    ...
    GARMENT_OBJ_PATH = f"/root/wyc/code/smpl2garmentcode2/AutoGarmentCode/output/CloSe/{sample_name}/design/design_sim.obj"
    PRED_SMPL_OBJ_PATH = f"/root/wyc/code/smpl2garmentcode2/AutoGarmentCode/output/CloSe/{sample_name}/smpl.obj"
    TARGET_POSE_NPZ = f"/root/wyc/data/CloSe/data/CloSe-Di/{sample_name}.npz"
    OUTPUT_DIR = f"/root/wyc/code/smpl2garmentcode2/AutoGarmentCode/output/CloSe/{sample_name}/driven/"
    PRED_SMPL_JSON_PATH = f"/root/wyc/code/smpl2garmentcode2/AutoGarmentCode/output/CloSe/{sample_name}/hybrik/smpl.json"
    ...
    print(f"load pred body:{len(pred_body_verts)} vertics, {len(pred_body_faces)} faces")
    ...
    print(f"[step one] zero gravity lose {zero_gravity_steps} steps")
    ...
    print(f"[step two] pose transition:{transition_frames} frames")
    ...
    for frame in range(transition_frames):
        ...
        # 每 20 帧更新一次绑定点索引，跟踪变形人体
        # if frame % 20 == 0:
        #     sim.update_attachment_body_idx(interp_body_verts)
    ...
    print(f"[step three]simulation:less than {max_sim_steps} steps")
    ...
    # 在正式仿真开始时更新一次绑定索引
    # sim.update_attachment_body_idx(target_body_verts)
    ...
    for step in range(max_sim_steps):
        ...
        if max_displacement < static_threshold and step > 50:
            print(f"sim convergence at {step} ")
            break

"""
服装生成结果评估 —— 严格对照论文第四章 §4.5.1 指标定义实现。

论文定义的五项指标：
  1. 分类准确率 (Meta Acc.)：meta 中 upper / bottom / 腰带(wb) / 连体(connected)
     四项分类参数相对数据集语义标注的预测正确率。
  2. 倒角距离 (Chamfer Distance, CD)，单位 cm，式(4-9) 含 1/2 系数：
         CD(P,Q) = 1/2 * ( mean_{p in P} min_{q in Q} ||p-q||
                          + mean_{q in Q} min_{p in P} ||q-p|| )
  3. F-Score：距离阈值 τ = 5mm 下精确率/召回率的调和平均。
  4. 物理模拟成功率 (SSR)：样板可成功缝合、稳定悬垂、无穿透/掉落的比例。
  5. 结构合法率 (Val. Rate)：输出可被解析、通过格式校验并由制版引擎成功加载的比例。

几何评估前先把两网格统一到同一坐标系（论文：基于 SMPL 骨骼的刚性对齐），
对齐后在两表面各均匀采样 10000 点再计算 CD 与 F-Score。

【已知限制】预测服装披附在 A-pose 的 SMPL 人体上，而 CloSe 真值 (canon_pose)
为规范化 T/star-pose。刚性对齐只能消除平移/旋转/整体尺度差，无法补偿这种
非刚性的姿态差异，因此本评估得到的 CD 绝对值会高于论文报告值（约几 cm 量级）。
要复现论文的 CD≈1.6cm，需在生成阶段令披附姿态与真值姿态一致（或用 SMPL-LBS
将预测服装重姿态到 canon_pose），这属于生成流程而非评估公式的范畴。
"""
...
def evaluate_meta_accuracy(design_cfg: dict,
    ...
    accuracy = sum(correct.values()) / 4.0
...
def rigid_align(pred_garment_v: np.ndarray,
    ...
    gt_canon = gt_data['canon_pose']
    gt_body = gt_canon[gt_labels == 1] # 筛选出 SMPL 人体皮肤点，后续用来定位人体中心、脚底高度
    gt_garment = gt_canon[~np.isin(gt_labels, NON_GARMENT_LABELS)]
    if len(gt_garment) == 0:
        gt_garment = gt_canon
    ...
    if len(gt_body) > 0:
      # 计算真值人体的基准位置
        gt_feet_y = gt_body[:, 1].min()
        gt_cx, gt_cz = gt_body[:, 0].mean(), gt_body[:, 2].mean()
    else:
        gt_feet_y = gt_garment[:, 1].min()
        gt_cx, gt_cz = gt_garment[:, 0].mean(), gt_garment[:, 2].mean()

    if pred_body_obj and os.path.exists(pred_body_obj):
      # 计算预测侧的基准位置
        bv = trimesh.load(pred_body_obj, force='mesh').vertices  # A-pose, 米
        pr_feet_y = bv[:, 1].min()
        pr_cx = (bv[:, 0].max() + bv[:, 0].min()) / 2.0
        pr_cz = (bv[:, 2].max() + bv[:, 2].min()) / 2.0
    else:
        pr_feet_y = aligned[:, 1].min()
...
def evaluate_single_sample(npz_path: str,
    ...
    # ---- 4. CD & F-Score ----
    gt_labels = gt_data['labels']
    gt_canon = gt_data['canon_pose']  # 米制规范坐标，作几何真值

    # angle = np.pi/4.0
    # angle2= np.pi/20.0

    # gt_canon[0,16,2] = -angle
    # gt_canon[0,17,2] = angle
    # gt_canon[0,16,1] = angle2
    # gt_canon[0,17,1] = -angle2
...
if __name__ == "__main__":
    ...
    parser.add_argument('--single', type=str, default="10062_6298",
                        help='single sample evaluation, pass sample name like 10001_1923')

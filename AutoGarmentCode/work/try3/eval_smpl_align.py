...
NPZ_PATH  = "/root/wyc/data/CloSe/data/CloSe-Di/10014_2464.npz"
SMPL_JSON = "/root/wyc/code/smpl2garmentcode2/AutoGarmentCode/work/try3/smpl.json"
GARMENT_OBJ = "/root/wyc/code/smpl2garmentcode2/AutoGarmentCode/work/try3/output/final_result.obj"  # Warp 驱动到 GT pose 后的网格
OUTPUT_DIR = "/root/wyc/code/smpl2garmentcode2/AutoGarmentCode/work/try3/align"
...
# ==================== 4. 加载服装并应用变换 ====================
print("\n" + "=" * 60)
print("4. 加载服装网格并修正初始高度")
print("=" * 60)

garment_mesh = trimesh.load(GARMENT_OBJ, process=False)
garment_v = np.array(garment_mesh.vertices, dtype=np.float64)
# 如果发现读出来的坐标在 100 左右，说明是厘米，需要除以 100
if (np.max(garment_v[:, 1]) - np.min(garment_v[:, 1])) > 5.0:
    garment_v /= 100.0  
...
# 真实衣服的最高点通常在肩膀位置，比头顶低大约 20~25 厘米
# 我们粗略把预测衣服的最高点，移动到预测人体头顶下方 22 厘米处
target_garment_top_y = pred_body_top_y - 0.22

# 当前预测衣服的最高点
current_garment_top_y = garment_v[:, 1].max()

# 计算高度差，强制上提或下拉
y_shift = target_garment_top_y - current_garment_top_y
garment_v[:, 1] += y_shift

print(f"  [修正] 已将服装 Y 轴平移 {y_shift:.3f} 米，使其最高点达到 {target_garment_top_y:.3f} 米")

# 应用 Kabsch 变换 (此时高度已经基本正确了)
garment_v_aligned = (garment_v @ R_mat.T) + t_vec
print(f"  garment 粗对齐后: Y=[{garment_v_aligned[:,1].min():.3f}, {garment_v_aligned[:,1].max():.3f}]")

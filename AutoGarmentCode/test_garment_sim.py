import os
os.environ['EGL_DEVICE_ID'] = '0'  # Set the GPU device ID for EGL
...
def get_command_args():
    ...
    parser.add_argument(
        "--body_path", "-b", type=str, help="Body measurements to use for the test garment."
    )
    parser.add_argument(
        '--pattern_spec', '-p', 
    ...
    parser.add_argument(
        '--body_name', '-bn', default='mean_all', help='Name of the body to use for simulation. Should correspond to a yaml file in the bodies folder. Ignored if --body_path is provided.')
...
if __name__ == "__main__":
    ...
    use_smpl_seg = True if 'smpl' in body_name else False
    
    if body_path is not None:
        print(f"Using body measurements from {body_path}")
        body_name = 'smpl'
        use_smpl_seg = True
    ...
    paths = PathCofig(
        ...
        body_name=body_name,    # 'f_smpl_average_A40'
        body_path=body_path,
        smpl_body=use_smpl_seg,   # NOTE: depends on chosen body model
        add_timestamp=True
    )

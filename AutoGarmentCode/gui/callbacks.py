...
class GUIState:
    ...
    def def_pattern_display(self):
        ...
        with ui.column().classes('h-full p-0 m-0'):
            ...
            with ui.image(
                ...
                ).classes(f'aspect-[{self.canvas_aspect_ratio}] h-[95%] p-0 m-0 relative')  as self.ui_pattern_bg:  
    ...
    def def_3d_scene(self):
        ...
        with ui.scene(
            ...
            self.ui_body_3d = self._create_body_3d_model()

    def _create_body_3d_model(self):
        """Create body mesh object for the current body selection."""
        mesh_path = self.pattern_state.get_body_mesh_relpath()
        ext = Path(mesh_path).suffix.lower()
        if ext in ('.glb', '.gltf'):
            return self.ui_3d_scene.gltf(mesh_path).rotate(np.pi / 2, 0., 0.).material(color='#000000')
        return self.ui_3d_scene.stl(mesh_path).rotate(np.pi / 2, 0., 0.).material(color='#000000')

    def _refresh_body_3d_model(self):
        """Reload body mesh in the 3D scene when body selection changes."""
        if self.ui_body_3d is not None:
            self.ui_body_3d.delete()
        with self.ui_3d_scene:
            self.ui_body_3d = self._create_body_3d_model()
        if hasattr(self, 'ui_body_3d_switch'):
            self.ui_body_3d.visible(self.ui_body_3d_switch.value)
    ...
    def def_body_file_dialog(self):
        ...
        async def handle_upload(e: events.UploadEventArguments):
            body_yaml = yaml.safe_load(e.content.read())
            param_dict = body_yaml['body']
            body_name = Path(e.name).stem

            self.toggle_param_update_events(self.ui_active_body_refs)

            self.pattern_state.set_new_body_params(param_dict, body_name=body_name)
            self.update_body_params_ui_state(self.ui_active_body_refs)            
            await self.update_pattern_ui_state()
            self._refresh_body_3d_model()

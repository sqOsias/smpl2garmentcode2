...
class GUIPattern:
    def __init__(self) -> None:
        ...
        self.body_file = None
        self.bodies_path = Path.cwd() / 'assets' / 'bodies'
        self.design_file = None
        self.body_name = 'f_smpl_average_A40' #'f_smpl_average_A40'
        self._load_body_file(
            self.bodies_path / f'{self.body_name}.yaml'
        )
    ...
    def _load_body_file(self, path):
        self.body_file = Path(path)
        self.body_name = self.body_file.stem
    ...
    def set_new_body_params(self, body_params, body_name: Optional[str] = None):
        self.body_params.load_from_dict(body_params)

        if body_name:
            self.body_name = body_name
            yaml_path = self.bodies_path / f'{body_name}.yaml'
            if yaml_path.exists():
                self.body_file = yaml_path

    def get_body_mesh_relpath(self):
        """Get static URL path to body mesh file for GUI 3D display.

        NiceGUI viewer supports STL/GLB. If only OBJ is available, convert it
        to GLB on demand and return the converted asset path.
        """
        stl_candidate = self.bodies_path / f'{self.body_name}.stl'
        if stl_candidate.exists():
            return f'/body/{stl_candidate.name}'

        glb_candidate = self.bodies_path / f'{self.body_name}.glb'
        if glb_candidate.exists():
            return f'/body/{glb_candidate.name}'

        obj_candidate = self.bodies_path / f'{self.body_name}.obj'
        if obj_candidate.exists():
            # Convert once and persist next to source OBJ.
            # Reuse the existing GLB in subsequent runs.
            if not glb_candidate.exists():
                mesh = trimesh.load_mesh(obj_candidate, process=False)
                mesh.export(glb_candidate)

            return f'/body/{glb_candidate.name}'

        # Fall back to the historical default mesh.
        return '/body/mean_all.stl'
    ...
    def _view_serialize(self):
        ...
        try:
            ...
            self.svg_filename = ''
            print('WARNING::Cannot serialize empty pattern!')
